...
import { getHiraganaRows, getAllHiragana, type KanaRow } from '~/data/hiragana'

const rows = getHiraganaRows()
...
<template>
  <div>
    ...
    <div v-if="!selectedRow">
      <div class="mb-6">
        <h1 class="text-2xl font-700 text-fg-strong mb-1">히라가나</h1>
        <p class="text-sm text-fg-muted">행을 골라서 5글자씩 배우거나, 전체 도전!</p>
      </div>

      <!-- All button -->
      <button
        class="w-full card p-4 mb-4 text-left cursor-pointer hover:border-ai/30 hover:shadow-glow-ai transition-all"
        @click="selectAll"
      >
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-sm bg-ai-light flex items-center justify-center">
            <span class="text-ai font-700 text-sm">46</span>
          </div>
          <div>
            <div class="font-600 text-fg-strong">전체 히라가나 도전</div>
